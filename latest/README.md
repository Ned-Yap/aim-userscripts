...
- [Latest - AIM Map Nav](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/latest/AIM_Map_Nav.user.js) — **NEW (dev-only)**: WASD pan / Q-E zoom (always-on) / Alt sprint (3x) / Space = zoom-to-fit site / Shift+Space = zoom in at cursor. Shift + other nav key bypasses Map Nav so existing macros (Shift+D Delete etc.) keep working. Ctrl + nav key passes to browser shortcuts. No prod counterpart yet.
