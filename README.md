...
| 2 | **AIM Map Styler** (outlines, buffers, KML shielding, Coverage Validator — Shift+O; Shift+K kicks a stuck map) — plus map overlays: **FAA sectional chart + vector airspace, USDA crop cover (dusted crops), Texas RRC wells/pipelines with pad points + operator lookup + Pad Recon, land-parcel owner lookup, TX districts/counties/survey grid, basemap switcher (fresh Esri/USGS imagery + dark map mode), color-coded terrain/elevation with contours + altitude legend** | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_SS_Outlines_Tampermonkey.user.js) |
...
| 7 | **AIM Site Setup Tools** — right-click any entity for inspector popup; SUM button → Summary panel (FP segments as first-class rows, **nested-asset columns** Parent / Root / Depth / Children / Nest path + `parent:` `root:` `depth:` search tags, inline edit Min/Max/AGL with formula support, bulk → AGL + bulk → Delta, **apply queue automates Percepto's entity editor** for the whole queue), 📊 site stats popup, 🗺️ **Site Setup Analyzer** (2D/3D KML export incl. Power Lines pulled from Map Styler); ground elevation per-entity with multi-point DEM sampling and a **team-shared elevation cache** via the data repo; **SOP Validators** (12 geometric site checks → drawn as map issues) and the **Airspace Validator** (live FAA data: controlled airspace, 3 NM manned-strip standoff, obstacles/turbines with fly-over clearance, LAANC ceilings, live TFRs auto-rechecked — violations drawn as issues, one-click GM creation at flagged obstacles). _(Shows in Tampermonkey as “AIM Site Setup Tools”; the underlying file is still `AIM_Copy_Asset_Name.user.js` so auto-update keeps working.)_ | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Copy_Asset_Name.user.js) |
...
| 10 | **AIM Performance Shield** (blocks session-replay + chat + weather network traffic; optional hide-satellite / ortho low-res for slow hardware) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Perf_Shield.user.js) |
| 11 | **AIM Mission Bank Tools** (SUM button → Mission Summary panel with sortable/filterable table of all missions on the site; click any row → drill-down with per-step instructions, ground elevation + AGL, inline altitude editor with batch commit, KML export of GPS pins; right-click any mission row → quick stats popup) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Mission_Bank_Tools.user.js) |
| 12 | **AIM Quick Mission Editor** (bulk-reorder mission instructions — Ctrl+Click drag handles to select, Enter to open move dialog) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Quick_Mission_Editor.user.js) |
| 13 | **AIM Power Line Editor** (⚡ button in map toolbar — drives Map Styler's vertex-edit / draw / delete-line / branch-from-vertex / snap-to-vertex modes for editing distro + trans KML files in-place; commits to GitHub) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Power_Line_Editor.user.js) |
| 16 | **AIM Map Nav** (keyboard map navigation — WASD pan, Q/E zoom, Alt = sprint, Space = fit-to-site; Shift+drag uses Leaflet's native box-zoom) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Map_Nav.user.js) |
| 17 | **AIM Issues** (🚩 button in map toolbar — flag manually-discovered issues with a polygon + note; full status workflow (Open → Ready → Resolved/Ignored, with re-open); GitHub-synced under your GitHub identity; affected-entities detection lists every Percepto entity under each polygon; dedicated panel with filter/search/sort and Google Sheets export) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Issues.user.js) |
| 18 | **AIM Bulk User Sites** (admin User↔Site batch-create page — ⚡ Bulk Fill / Shift+B opens a panel to paste or search Users + Sites, Add-all, favorites, presets; Role defaults to Percepto Admin) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Bulk_User_Sites.user.js) |
| 20 | **AIM Video Validation** (Mission Playback page — snapshot strip in flight order with S# badges, click a thumbnail to seek the video to that shot, playhead highlights the current shot, shot card with planned-vs-actual heading / camera angle / altitude / drone-vs-nav offset, plan steps + actual camera footprints drawn on the map with planned look-points, whole-mission-group view with per-flight colours and a flight list that opens sibling flights, time bar with ±10/30 s skip and jump-to-time; experimental Adjust panel (off by default) to nudge steps and save) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Video_Validation.user.js) |
| 19 | **AIM Defaults** (smart site navigation — land on your default section, keep your section when switching sites — plus auto map-layer defaults / Feeder Line bring-to-top on each new site; all configurable in the Control Panel) | [Install](https://raw.githubusercontent.com/Ned-Yap/aim-userscripts/main/AIM_Defaults.user.js) |
