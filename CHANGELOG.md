...
## 2026-09-25 — 🎥 Flight Checks now runs the playback page's engine: AIM Fleet Tools v0.58 (latest — dev only)

Fleet-wide Flight Checks (Fleet Tools → 🎥 Flight Checks) measures every flight the way the Mission Playback card does, so what the card flags, the fleet view flags — no more "the card sees S87 102 ft off but Fleet says the flight is clean". Per flight it now reads the as-flown plan, the picture records **and the flown positions** (which plan step was executing when), joins each picture to that step (pose override when the log is sparse), numbers shots S1… exactly like the playback page, and computes the **planned-vs-actual look-point gap** (where the camera ray meets the terrain, via Percepto's DEM) next to heading / camera / altitude / off-station. "No picture" comes from the flown slice, not from whatever the old matcher managed to align. Flights whose flown slice has no planned snapshot (Jump-To-Alert, manual) are listed as **manual / alert** and never flagged; a picture taken while a navigate step was executing is shown as a manual shot, not an error. Limits (now incl. look-point, default 50 ft) apply at render — change a number and the cached flights re-score instantly, no re-fetch. The flags view adds single obvious hits (a shot more than twice over a limit) and a per-drone "looks ~N ft from the planned spot on average" pattern for IT. Cache rebuilt (coreVer 3); first run re-reads flights. Feature [#267](https://github.com/Ned-Yap/aim-userscripts-issues/issues/267).

## 2026-09-25 — 📬 Site Watch daily digest is back, as a Slack DM: AIM Site Watch v0.32 (latest — personal)

The daily change digest removed on 2026-08-27 returns, but it now goes to the owner's Slack DM instead of CSM-Site-Issues, so the channel stays lifecycle-only. Same report as before (per-site rollup parent + threaded detail, setup and missions), fired once per PT day at the hour set in the Control Panel ("Daily digest → my Slack DM at this hour", default 6 pm PT), also on quiet days when no site was due. On first run it baselines silently, so nothing from the gap since August is replayed. The manual button is relabelled "Post digest to the CHANNEL now" to make clear it is the one path that still posts publicly.

## 2026-09-25 — 🔒 Site Watch is now owner-only: AIM Site Watch v0.29 (latest — personal)

Site Watch polls every site and commits under whatever GitHub PAT is in the Control Panel, so it must only ever run for its owner. From this version the script verifies the PAT's GitHub login once per token; any login other than the owner's is locked out — master forced off, nothing polled, nothing written, and the Control Panel section shows a single lock notice asking for the script to be uninstalled. Unverified (no token, network failure) also does not run. If you are not the owner and have this installed, remove it from Tampermonkey.

## 2026-09-25 — 🚫 Site Watch: no more phantom "(non-structural change)" rows: AIM Site Watch v0.28 (latest — personal, dev only)

The first automatic Fleet Agent DM (site 1467, 17:04) explained a change that never happened. A second Site Watch install had already recorded the real change two hours earlier; this install's local hash was stale, the repo snapshot was already current, and the value-level diff correctly found nothing, so the watcher wrote a nameless "(non-structural change)" row that the explainer then narrated. Now: if the repo's previous snapshot already equals the current state, the watcher adopts the hash and writes nothing (no row, no snapshot, no HOT). If the snapshot genuinely differs but the reporter has no rule for the field, the row names the entity and the exact fingerprint path that moved instead of a placeholder. Agent side (aim-userscripts-data agents v0.2): a batch made only of placeholder rows is skipped before any model call, and the prompt says such rows carry no information.
...
## 2026-09-25 — 🧑‍✈️ Pilot Utilization: AIM Fleet Tools v0.46 (latest — dev only, feature #274)

New section on the fleet panel. Pick sites in **Fleet Data**, set the window (last 28 days by default), shift length (8 h) and time zone (Central), then **▶ Pilot utilization**. Every flown flight becomes a takeoff → landing interval for the pilot who launched it, split at local midnight. Because pilots fly one-to-many, a pilot's **air time** for a day is the *union* of their intervals: two drones up for the same 30 minutes count as 30 minutes once, never 60. **Drone-hours** (the plain sum) sits beside it, and **Drones ⌀** = drone-hours ÷ air time shows how many drones a pilot typically keeps up at once, with an hours-at-1 / 2 / 3 / 4+ drones breakdown. **Util %** = air time per active day ÷ shift length. Four sortable tabs: **Pilots** (active days, flights, flights/day, air/day, util, best and lightest day), **Pilot-days** (one row per pilot per day with first takeoff, last landing and span — sort by air time to see who flew the most or least in a day), **Dates** (per calendar day: pilots active, total air, most/least), **Flights** (every flight, click a row to open its playback, "Overlapping" = how many of the pilot's other flights were up at the same time). Click any header to sort; **📊 Copy → Sheets** / **⬇ CSV** export the current tab in the current sort. Read-only.
...
## 2026-09-18 — 📌 GM Stamper: AIM Site Setup Tools v4.297 (latest — dev only, feature #271)

General Markers all need a unique name, and the native flow makes you pick the type, type a name and re-enter the notes for every single one. The new **📌 GM Stamper** (purple pin in the map toolbar, or its own Control Panel card) fixes that with **templates**: give one a label, a **name pattern** with `#` (e.g. `Gate #` → `Gate 1`, `Gate 2`, … the `#` always becomes the next free number on the site, checked against every entity name so Percepto never rejects it), the **marker type** (= the icon: general / tower / hazard / building / pole, or a custom value), the **notes**, a height in ft, and optionally a start number and zero-padding. **Arm** the template and every click on the map creates a real General Marker right there — it shows up immediately as a labelled preview pin, and a page reload shows it as a native Percepto marker. **↩ Undo last** deletes the most recent one (Delete Guard banks it first). Clicks that turn into a pan don't place anything; if you'd rather keep plain clicks free for Percepto, turn on **Require ALT+click** in the card. Templates are saved per user (they carry across prod and QA) and **⬇ Copy JSON / ⬆ Paste** shares a set with a coworker. Full mode only; create-only (never edits existing entities).

**v4.298 — Bulk → 📌 GM.** In the SUM panel, filter (e.g. Unshielded), select the rows, then **Bulk → 📌 GM**: pick a template and one General Marker is created at every selected row's centroid (the source assets are untouched). Rows that already have a GM within 50 ft start unchecked so a re-run only adds the new ones. The name pattern now also accepts `{name}` for the source entity's name (e.g. `Unshielded {name}`), numbering only if that name is taken. **↩ Undo batch** deletes everything the run created.

**v4.299 — ⭕ Radius rings per template.** Each template can draw a **radius ring** (distance in mi or ft, colour, opacity, optional N/S/E/W cross) around every marker on the site that matches it (same type + name pattern), so the "power lines within 0.5 mi on 3 sides" check for an unshielded asset is a glance at the hazard marker's ring. Toggle per template with the ⭕ button on its row or in the editor (changes preview live on the map while you edit); **⭕ Rings ON/off** in the panel header hides them all. Rings rebuild automatically on reload, site change, and after every stamp/undo, and they cover matching markers created natively too.

**v4.300 — rings for markers named your way + live ring while editing a GM.** The ring editor gained **Also match**: comma-separated terms (e.g. `not shielded`) so any existing marker whose name contains one gets the ring regardless of how it was named, with an **any marker type** option. Opening a template now previews its rings immediately (pattern / type edits re-match live too). And while you edit an **existing General Marker in Percepto's own form**, a dashed preview ring appears on it the moment its name matches a ring template, so you can size the 0.5 mi check before saving.

## 2026-09-18 — 📊 Entities → Sheets from the fleet site picker: AIM Fleet Tools v0.41 (latest — dev only, feature #270)

Pick sites in **Fleet Data** (tick a whole client, e.g. every Exxon site), then use the new **Entities:** row: **📊 Copy → Sheets** puts one row per entity of every picked site on the clipboard as a real table (paste into Google Sheets / Excel — the Site column is a link), or **⬇ CSV** downloads the same thing. Per-type checkboxes (Assets / FFZ / FP / NFZ / Markers / Base / Safe) choose what goes in; they're remembered. Columns are everything Percepto stores for the ticked types — name, ID, type, subtype / equipment / state, description, GPS + vertices + area, FFZ/NFZ altitude bands, flight-path length + altitudes, asset POI / pole / elevation fields, base-station drone — and "Key: value | Key: value" descriptions (the Exxon asset convention) are split into their own **Desc:** columns so the client's asset ID is a sortable cell. Optional extras: **coordinates** (every vertex, FP arcs, polygon WKT) and **raw JSON**. Read-only; sites are fetched one at a time with progress + Abort.
...
## 2026-09-09 — 📎📐 Fleet Tools v0.18: KML layers + cross-reference engine (latest — dev only, feature #255)

Google-Earth-style overlay layers on the landing map, plus the analysis they exist for. **📎 KML Layers:** ⬆ Load any .kml (points / paths / polygons — KMZ must be unzipped first). Layers are session-only until 💾 saves one to the private data repo (`fleet-kml/`, create-only), where it persists for every session (☁). Per-layer show/hide, color, line width, polygon fill, opacity — remembered per user — plus search across layer AND feature names with 🎯 fly-to, and ✕ removal (repo deletes need a second confirming click). Rendered through the raw SVG overlay as merged paths, so even big networks stay cheap. **📐 Cross-reference:** pick a source KML layer and a target — **Site FFZs + FPs (existing coverage)** or another KML layer — with two editable bands (≤50 ft / ≤200 ft default). Lines are sampled (~10 ft adaptive) and every stretch classified: green ≤ band 1, amber ≤ band 2, red beyond, drawn on the map with a stretch list (🎯 per row) and a copyable report of lengths, miles, and percentages — including the headline number: **how much of the client's water lines is inspectable from existing sites without building new areas** (cumulative ≤200 ft). Coverage counts only accessible, non-⊘'d (and optionally Production-only) sites; point features get per-band counts. Segment/polygon math per the engraved rules, cooperative yields, abortable.
...
## 2026-09-09 — ⚠ NEW SCRIPT: AIM Fleet Tools v0.1 — fleet-wide overlap sweep on the landing page (latest — dev only, feature #250 layer 1)

Second of the three cross-site overlap defense layers, and the first tool that lives OUTSIDE any site: a **⚠ Fleet Tools** button appears on the sites-select landing page (bottom-right, before entering a site). **⚠ Overlap Sweep** checks EVERY pair of sites in the environment: Site Watch snapshot bboxes prefilter candidate pairs (same incremental sha-diff index as Site Diff's neighbor overlay), live `/map_objects/` fetches supply current geometry for candidates only, and the shared segment-to-segment / polygon-containment math flags pairs whose FFZs / flight paths / assets come within the threshold (default 200 ft, editable, per-class checkboxes). Results: conflicting site pairs sorted closest-first, expandable to per-conflict rows (entity ↔ entity, distance, coordinates) with open-site links, a copyable full report, and the last sweep cached so it's there on reload. Duplicate/OFFLINE site copies get the same treatment as in Site Diff v0.82: **⊘ off** a site from any pair row and it's hidden and excluded from future sweeps (persisted per server, re-enable chip + named in the report — never silently skipped). Sites with no snapshot fall back to a `/sites/` center + 1 mi margin or are listed UNCHECKED. **📊 Fleet Metrics (bones)** — per-site FFZ/FP/asset counts straight from the snapshot index, client column when detectable, Copy CSV; minimal on purpose, the section framework is the point. PAT arrives via the Control Panel's TOKEN_VALUE broadcast (verified working on the landing page). Layer 3 (SOP Validator check #13) still to come.
...
## 2026-08-30 — 📥 Site Setup Tools v4.251: Asset Importer ✎ Edit — reshape any selection shape (latest — dev only, feature #249)

New **✎ Edit** tool in the importer's selection row: while armed, every shape shows its points. **ALT+drag a vertex dot** to move it, **ALT+click a small midpoint dot** to insert a new vertex there (keep dragging to place it), **ALT+right-click a vertex** to delete it (shapes keep a 3-point minimum), and **ALT+drag a circle's white edge dot** to resize its radius — selection counts and the Shapes list update live through every tweak, and the numbered handles still move whole shapes. Circles have no vertices, so each circle's row in the Shapes list gains a **⬡ convert** button that turns it into a 24-point polygon in place (same number, same selection) — after that you can add/move/delete its points like any lasso or uploaded KML shape. Works on every shape kind: drawn lassos, ⬆ Shape (KML) polygons, and converted circles, in both create and 🗑 Delete mode.

## 2026-08-28 — 📥 Site Setup Tools v4.247: Asset Importer — CSV/KML → spatial select → bulk-create, + guarded asset bulk delete (latest — dev only, feature #249)

New tool in the ⊕ Site Setup Generator (📥 Import button, own Control Panel card "Asset Importer"). Load a region-wide **CSV or KML** (file picker or drag-and-drop onto the panel): a column-mapping UI auto-guesses Name/Lat/Lng/Type (delimiter auto-detected, quoted fields handled; the mapping is remembered per vendor header layout, so the same export maps instantly next time). Points stage on the map as lightweight orange markers — **nothing is written** — viewport-culled so thousand-row region lists stay smooth. Select the subset the site's drone can actually cover: **⭕ Circle** (ALT+click centers it; editable radius in mi/ft), **🖊 Lasso** (ALT+drag a freehand loop), **☝ Pick** (ALT+click toggles one) — all additive, live count in the panel. CSV type values map to Percepto subtypes through an editable, remembered dictionary (unmapped values block Commit loudly); a **duplicate guard** flags staged points that name-match an existing asset or sit within an editable radius (default 50 ft, CP card) of one — flagged points are skipped unless explicitly included. **✓ Create selected** rides the ➕ Create-asset rail: create-only squares (size CP-editable), honors the modal's Dry-run checkbox, downloads a backup manifest first, verifies by refetch, keeps failures staged + retryable, and draws the familiar green ghosts. ↩ Undo last import deletes exactly what it created. The inverse: **🗑 Delete mode** flips the same circle/lasso/pick tools to select EXISTING assets (red) → mission-reference scan + full backup download + Delete Guard detection → ack checkbox + **slide-the-bar-and-HOLD-5-seconds** ceremony (release cancels). Deletes go through the normal API so **Delete Guard banks every one to its 24h undo ring**; if Delete Guard isn't detected the delete is blocked unless a separate override is ticked. ↩ Undo this batch re-creates the deleted assets from the tool's own bank (new ids — mission references don't rebind, and the preview warns which missions reference what). Replaces the manual CSV→KML→external-circle-script→hand-entry workflow.

## 2026-08-28 — ⬡ Site Diff v0.80: neighboring-site overlay + cross-site conflict check (latest — dev only, feature #250 layer 2)

First of the three cross-site overlap defense layers. New "Neighbor sites" section in the Site Diff Control Panel card: **Show neighboring site setups** overlays every OTHER site's FFZs / flight paths / assets within an editable display radius (default 1000 ft) of the open site — each neighbor drawn dotted in its own color (color = which site, legend in the ⬡ panel). Any foreign entity within the **conflict threshold** (default 200 ft, editable, per-class checkboxes) of a local entity is flagged: red pin at the closest approach, plus a panel listing every conflict with distances and click-to-zoom. All proximity math is segment-to-segment / polygon-containment — never vertex-to-vertex. Neighbor discovery is the hybrid model from the design doc: Site Watch snapshot bboxes (private data repo, PAT) prefilter candidates incrementally by git sha, live `/map_objects/` supplies current geometry for the real math; sites with no snapshot fall back to a `/sites/` center + 1 mi margin, and sites with neither are listed as **UNCHECKED** in the 📋 report — never silently skipped. QA and prod stay fully isolated (env-keyed index, same-server neighbors only). Layers 2 of 3 still coming: AIM Fleet Tools landing-page sweep + SOP Validator check #13.
...
## 2026-08-28 — ⇅ v2.89: add pads by M2, Ctrl+M2 remove, full map-gesture editing (Mission Bank Tools latest — dev only)

The ⇅ panel is now a complete macro composer driven from the map. While it's open: **M2 an outside pad** (any pad not in the macro) adds it — its steps pull from its micro mission on Apply, it appends at the end with a green ＋ badge, and the row shows "＋NEW" with the micro's step count (a pad with no micro is refused with a clear message). **Ctrl+M2 any member pad** (badge or the pad ring itself) deletes it — same as the row ✕ — and M2 on a deleted pad's grey ✕ badge restores it. **Plain M2 a numbered badge** still opens the renumber box: type the new position and everything after shifts down one. Apply handles reorder + deletions + additions in one save, with the hard step accounting extended to expect exactly the added micros' steps and the removed pads' steps. Ctrl+M2 on a green ＋ badge discards the pending addition.

## 2026-08-28 — ⟳ v2.88: the last two converge — signature noise eliminated (Mission Bank Tools latest — dev only)

The v2.87 diagnostic caught them: both stuck pads failed on **navigate** steps from old micros (mission ids 4762 / 21785) whose extra_options lack the `pitch` key that every modern save adds by default — so a full extra_options comparison could never converge. Signatures now compare only the tuning-relevant fields per step type: snapshots keep `pitch` (the pilot points the camera), navigates keep `shouldUseFreezoneMinAlt` (altitude behavior), and everything else is save-path churn, excluded. The diagnostic also leveled up: a stuck pad now prints its micro signature NEXT TO the macro's counterpart at the same coordinates, so any future mismatch shows the exact differing field in one console line.

## 2026-08-28 — ⟳ v2.87: rounding-proof signatures + self-diagnosing diff (Mission Bank Tools latest — dev only)

Nine of the eleven stuck macros converged on v2.86; two still flagged. Suspected cause: Percepto stores coordinates as 1e-7-degree integers, and its save path can truncate where the raw float rounds up at the seventh decimal — a one-digit signature mismatch that never converges. Signatures now use 6 decimals (~11 cm, still far tighter than any real step distinction), and null-valued extra_options fields are stripped (null-vs-absent round-trip noise). And the diff now diagnoses itself: any pad that flags UPDATED logs the exact unmatched step signature(s) to the console — or says "all steps present but not contiguous" — so a stuck pad names its own root cause instead of needing another guessing round.

## 2026-08-28 — ⟳ v2.86: the never-converging UPDATED loop fixed (Mission Bank Tools latest — dev only)

Live catch: 11 macros re-merged five times kept flagging UPDATED after every reload. The saves were fine — the **diff** was wrong. It compared each micro against the macro's geometry-sliced pad block, and on tight Permian sites a micro step that strays near a neighboring pad (the log's own "nav within 150 ft of a neighboring pad" case) gets filed under the neighbor — so the comparison could never match, no matter how many re-merges ran. New idempotent diff: a pad is **in sync iff its micro's exact step sequence appears as a contiguous run inside the macro body** (signatures cover type, values, coordinates, and extra_options — so pitch tuning now correctly flags too). Right after a re-merge that's true by construction; the stuck 11 should read "in sync" immediately since their saves already landed. Also new: a cross-pad overlap guard — if a pulled micro's steps also live inside a KEPT pad's block, the save refuses (pull both pads or neither) instead of silently duplicating those steps.

## 2026-08-28 — ⇅ v2.85: delete pads right in the reorder panel (Mission Bank Tools latest — dev only)

Deleting a pad from a macro shouldn't require the 🪄 Step Optimizer's full legal-route analysis — that tool checks every step's logic before it lets you do anything. The lightweight ⇅ pad-order panel now handles it: every row has a red **✕** that marks the pad for deletion (its map badge turns into a grey ✕ live), a "Deleting N pad(s)" list shows what's going with an ↩ restore per pad, and Apply removes those pads' steps while applying whatever reorder you made — one save, same rails (backup, hard step accounting that expects exactly the deleted steps gone, verify-by-refetch). Micros untouched; ↺ Reset also restores deletions; deleting every pad is refused. ✂ Cut in 🪄 stays for step-level surgery (single navs/snapshots).

## 2026-08-27 — ⟳ v2.84: Re-merge ALL (Mission Bank Tools latest — dev only)

**⟳ All** joins ✂ All and 🪄 All in the 🧩 legend header: every macro on the site diffed against its pads' current micros in one review — per macro, how many pads are UPDATED (with the changed pads and their step deltas listed), in sync, or missing a micro. Macros with changes come pre-ticked; apply runs sequentially with per-macro verify and a single combined backup JSON for the whole batch (instead of one download per macro). Only UPDATED pads are pulled — in-sync and no-micro pads keep their current steps — and as always: pad orders, names, and ids unchanged. The single-macro ⟳ and ⟳ All now share the same plan/save core.

## 2026-08-27 — v2.83: floating panels drag + resize (Mission Bank Tools latest — dev only)

The tool panels are now movable: drag anywhere on the header (buttons and links still click through), resize with the native bottom-right handle. Applies to ⟳ Re-merge, the 🪄 Step Optimizer panel, the 🪄 All sweep report, and ✂ Split. The 🪄 panel re-renders on every checkbox tick, so it remembers where you dragged it and what size you made it — including across close/reopen — and only records a size when you're actually dragging the resize handle (content growth never freezes it).

## 2026-08-27 — ⟳ v2.82: re-merge a macro from its micros (Mission Bank Tools latest — dev only, feature #248)

Closes the loop on the micro-first workflow. Pilot feedback lands in the micro missions — they're the source of truth — but refreshing the macro used to mean re-lassoing everything and deleting the old macro over the name collision. New **⟳** on every macro row in the 🧩 legend: rebuilds the macro from its pads' current micros with the **same pad order, same name, same id**. The review panel lists the pads in visit order with each one's matched micro and a live diff — *in sync*, *UPDATED* (with nav/snap/step counts), or *no micro — keeping current steps*. Changed pads come pre-ticked; untick any pad to keep the macro's existing steps for it. The rebuild is exactly what a hand re-merge produces (takeoff + each micro's body in order + returnHome), and Apply rides the usual rails: JSON backup, in-place save, verify-by-refetch of pad order and step count. Interleaved facility pads consolidate at their first slot — run 🪄 after to re-optimize.

## 2026-08-27 — 🪄 v2.81: snapshot pairings are pilot-tuned — moves strictly opt-in (Mission Bank Tools latest — dev only)

Doctrine hardening from the field workflow: snapshot⇄nav pairings aren't guesses to optimize — they're **calibration data**, dialed in over one or two feedback flights (pilot: too close / too far / left / right / high / low / empty). A re-home that geometry rates "slightly better" silently destroys that tuning and restarts the dial-in from scratch. So the tool now NEVER moves a snapshot by default: every standoff suggestion renders info-only — distance, ladder tier, and how much closer the alternative is — with every checkbox starting UNCHECKED (previously too-close ones were pre-ticked). The panel says so in place. Reordering remains fully allowed and is structurally safe: snapshots ride their nav through every solver pass, so changing the order never changes a pairing.
...
## 2026-08-21 — 🖊 Lasso branch-walk order + 🧩 badge order fix (Mission Bank Tools latest v2.62 — dev only)

Two ordering fixes from live use. **(1) Lasso order doctrine update**: the order no longer accounts for RTB/battery at all (the 🔋 flight-break annotations remain, but they never shape the order). The order is now always **furthest→nearest, branch-coherent**: at a Y in the route it finishes every pad on one side before jumping to the other — even when the other side holds a deeper pad than the next pad on the current side. Mechanically each next pad is the one sharing the **deepest route junction** with the current pad (ties within 500 ft go to the deeper pad), computed on the same verified legal-route graph as before. **(2) 🧩 macro badge numbers could disagree with the macro's real step order** on adjacent pads: any nav within 150 ft of a pad ring counted as "visiting" it, so an approach/transit nav skimming the *neighboring* pad's halo stole its visit slot — and no amount of re-saving the merge could fix the badges, because the mission's step order was right all along. Visit order now goes to the first step actually **inside** each pad's ring (the loose 150-ft rule remains only as a fallback for pads a mission never enters, and still governs coverage membership). When the correction reorders anything, a `[macros]` console line names the mission.

## 2026-08-20 — 🔋/🧩 route solver — big-site speedup (Mission Bank Tools latest v2.61 — dev only)

Site 1386 took **50 s** for the 🔋 Range solve and another **121 s** for the 🧩 coverage audit, freezing the tab (long enough that Percepto's radar-status WebSocket watchdog complained). Four fixes, all producing **identical routes/classifications**: (1) the legal-route graph and full solve are now **cached per site geometry** — 🔋, 🖊 Lasso, 🧩 audit (which re-solved from scratch AND rebuilt the graph once per macro), 📋 report, and 👁 preview routing all share one solve instead of ~10 independent rebuilds; any entity edit invalidates it automatically. (2) Dijkstra now runs on a **binary heap** instead of a linear-scan queue (verified identical distances, big win on dense visibility graphs). (3) **Convex FFZs skip per-pair segment sampling** entirely (geometry guarantees the answer). (4) Concave-ring visibility probes now test **middle-out**, so failing pairs exit on the first probe. The `[range]` log line now includes a sparse/dense/classify timing breakdown — if a site is still slow, that line says exactly where.

## 2026-08-20 — NFZ builder round 2: exact shapes, commit placeholders, ⚠ Narrow check (Asset Inspector latest v4.246 — dev only)

Three fixes from the first live test. **(1) The shape now follows your trace exactly** — the outward-grow borrowed a pad-offset helper whose "outward" test is only right for convex shapes, so concave traces (L/U shapes) had notch corners pushed the wrong way; the NFZ tools now use a winding-aware offset that's correct on any shape (node-tested on concave rings). **(2) Commit placeholders**: Percepto doesn't render a new NFZ until you reload, so a successful commit now leaves each created NFZ on the map as a **solid red locked shape** (like committed FFZs) until you reload — they survive closing the modal and clear on site change. ↩ Undo removes its placeholders too. **(3) ⚠ Narrow check**: new yellow button in the ⬠ NFZ Draw panel paints **every part of every NFZ that is narrower than the Min size** — drafts, this session's commits, AND the site's existing live NFZs — catching a thin notch or tab sticking off an otherwise-large NFZ that a simple min-size check can't see (morphological analysis; a zone that's entirely under min gets fully outlined). Click again to clear. Fresh drafts are auto-checked at finish and warn in the toast.
...
## 2026-08-11 — Mission Bank Tools latest v2.51–v2.54: ⧉ Duplicate missions

- **Popover is a mini-panel now (v2.54)** — top-right ✕ to close, drag it anywhere by the header, resize from the bottom-right corner. It no longer closes when you click elsewhere on the page.
