...
- **Look:** purple polygon + a shield-with-✕ marker. Status shows via dash pattern; when **approved (resolved) it stays fully visible** — solid purple outline + a green ✓ chip on the shield — as a permanent "known-accepted unshielded section" marker. M1 can still temp-hide it for the session.
